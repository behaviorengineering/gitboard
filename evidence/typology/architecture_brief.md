...
|-------|-----------|-------|
| `board` | Manages the visual representation and state of project boards for users. |  |
| `gitboard` | Orchestrates the core CLI application, including orchestration, server, and observability. |  |
| `dashboard` | Provides a web-based interface for interacting with project data and management tasks. |  |
...
Manages the visual representation and state of project boards for users.

- Packages: ./internal/board, ./internal/config, ./internal/llm
- Surfaces: _(none)_
- Programs: _(none)_

#### `gitboard`

Orchestrates the core CLI application, including orchestration, server, and observability.
...
Provides a web-based interface for interacting with project data and management tasks.
...
Interfaces with the local filesystem to inspect and manage git worktrees and repositories.
...
Automates the decision-making process for cleaning up stale project branches and resources.
...
Communicates with remote git providers to fetch and analyze repository state.
...
Analyzes project state to suggest necessary maintenance and cleanup actions.
...
- `gitboard`: SliceBinding gitboard -> board missing but cross-slice import exists (cmd-gitboard -> internal-config)
- `gitboard`: SliceBinding gitboard -> board missing but cross-slice import exists (cmd-gitboard -> internal-llm)
- `gitboard`: SliceBinding gitboard -> board missing but cross-slice import exists (internal-server -> internal-config)
- `gitboard`: SliceBinding gitboard -> board missing but cross-slice import exists (internal-syncproj -> internal-config)
- `localgit`: SliceBinding localgit -> gitboard missing but cross-slice import exists (internal-localgit -> internal-cliexec)
- `pruneagent`: SliceBinding pruneagent -> board missing but cross-slice import exists (internal-pruneagent -> internal-llm)
...
- `triage`: SliceBinding triage -> board missing but cross-slice import exists (internal-triage -> internal-config)
- `triage`: SliceBinding triage -> board missing but cross-slice import exists (internal-triage -> internal-llm)
